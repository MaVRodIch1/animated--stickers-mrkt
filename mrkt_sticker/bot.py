#!/usr/bin/env python3
"""
MRKT Animated Sticker Bot — Telegram bot for on-demand sticker generation.

Commands / Команды:
  /start            — welcome / приветствие
  /sticker <slug>   — animated sticker (fallback to static) / анимированный стикер
  /animated <slug>  — explicitly animated sticker / анимированный стикер
  /static <slug>    — static PNG sticker / статический стикер
  /list             — list collections / список коллекций
  /price <slug>     — text price response / текстовый ответ с ценой

Setup / Запуск:
  1. pip install -r requirements.txt
  2. Ensure ffmpeg is installed for animated stickers
  3. Create .env file:
...
# Try to import animated sticker generator
try:
    from sticker_image import generate_sticker as generate_animated_sticker
    HAS_ANIMATED = True
except ImportError:
    HAS_ANIMATED = False
...
# ─── Генератор статических изображений ──────────────────────────
...
def generate_sticker_image(col):
    """Генерирует PNG-стикер 512x512 для коллекции (статический)."""
    slug = col.get("slug", "???")
...
async def cmd_start(message: types.Message):
    animated_note = "\n🎬 Стикеры теперь <b>анимированные</b>!" if HAS_ANIMATED else ""
    await message.answer(
        "🎁 <b>MRKT Animated Sticker Bot</b>\n\n"
        "Генерирую анимированные стикеры с ценами подарков Telegram!\n"
        f"{animated_note}\n\n"
        "📋 <b>Команды:</b>\n"
        "<code>/sticker PlushPepe</code> — анимированный стикер с ценой\n"
        "<code>/animated PlushPepe</code> — явно анимированный стикер\n"
        "<code>/static PlushPepe</code> — статический стикер (PNG)\n"
...
async def cmd_sticker(message: types.Message):
    """Generate sticker — tries animated first, falls back to static."""
    args = message.text.split(maxsplit=1)
    if len(args) < 2:
    ...
    wait_msg = await message.answer("⏳ Генерирую анимированный стикер...")

    try:
        # Try animated first
        if HAS_ANIMATED:
            try:
                webm_data = generate_animated_sticker(col, ton_usd, fmt="WEBM", gift_img=None, animated=True)
                sticker_file = BufferedInputFile(webm_data, filename=f"{col['slug']}_mrkt.webm")
                await message.answer_sticker(sticker=sticker_file)
                await wait_msg.delete()
                return
            except Exception as e:
                log.warning(f"Animated sticker failed: {e}, falling back to static")
...
@dp.message(Command("animated"))
async def cmd_animated(message: types.Message):
    """Explicitly generate animated WebM sticker."""
    if not HAS_ANIMATED:
        await message.answer("❌ Анимированные стикеры недоступны (sticker_image модуль не найден)")
        return

    args = message.text.split(maxsplit=1)
    if len(args) < 2:
        await message.answer("Использование: <code>/animated PlushPepe</code>", parse_mode="HTML")
        return
    ...
    try:
        ...
        log.error(f"Animated sticker error: {e}")
        await message.answer(f"❌ Ошибка генерации анимации: {e}")
    finally:
        try:
            await wait_msg.delete()
        except Exception:
...
async def cmd_static(message: types.Message):
    """Explicitly generate static PNG sticker."""
    args = message.text.split(maxsplit=1)
    if len(args) < 2:
        await message.answer("Использование: <code>/static PlushPepe</code>", parse_mode="HTML")
        return
    ...
    wait_msg = await message.answer("⏳ Генерирую статический стикер...")
    try:
        png_data = generate_sticker_image(col)
        photo = BufferedInputFile(png_data, filename=f"{col['slug']}_mrkt.png")
        await message.answer_photo(photo, caption=f"🎁 {col['slug']} — {col.get('floor_price', 0):.2f} TON")
...
async def main():
    log.info("🚀 MRKT Animated Sticker Bot запущен!")
    if HAS_ANIMATED:
        log.info("🎬 Animated sticker support: ENABLED")
    else:
        log.info("⚠ Animated sticker support: DISABLED (import failed)")
